...
- **input-validation との責務分担**: 入力検証の欠如を見つけても、攻撃経路が説明できない非悪意の不正値への防御は logic-reviewer（input-validation）の担当。本エージェントは攻撃経路が説明できるものを担当し、重複しそうな指摘はメインで統合される前提で自分の観点に集中する。
