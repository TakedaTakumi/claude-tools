...
- **input-validation との責務分担**: 入力検証の欠如は未検証の値の**到達先**で切り分ける。インジェクションのシンク・認証/認可の判定・機密の読み出しに到達するものが本エージェントの担当で、計算・永続化・表示での値の破壊は logic-reviewer（input-validation）の担当。到達先が特定できない検証欠落は、重大度を低めに付けたうえで本エージェントからも報告する（双方が「相手の担当」と判断して落とす事故を防ぐ）。
