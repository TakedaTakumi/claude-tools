...
- 検証の配置設計（入口で一度検証し、内部では検証済みの型で扱う構造になっているか。検証が欠落している箇所と、同じ検証が複数箇所に散在・重複している箇所の両方を見る）
- 不正値を検知した後の振る舞いが仕様と整合しているか（即時拒否 / デフォルト値へのフォールバック / 例外送出 / サイレント継続のいずれか。特にサイレント継続や暗黙の型強制によって不正値が黙ってデータを破壊していないか）
- スキーマバリデータ（zod / pydantic / JSON Schema 等）の活用度（`z.any()` 等での素通し、スキーマを使わない手書き検証がコードベースに散在していないか）
...
差分で追加・変更された「外部入力を受け取る関数・エンドポイント」（公開 API のハンドラ、CLI 引数のパース処理、設定読込、外部 API 応答のパース等）を列挙する。列挙したそれぞれについて、入口での検証の有無と、不正値検知後の振る舞い（拒否・デフォルト値・例外・サイレント継続）が仕様や既存の挙動と整合しているかを評価する。
