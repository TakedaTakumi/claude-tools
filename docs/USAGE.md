...
収録している各コマンドの使い方・引数・実行例をまとめる。汎用コマンド11個と、レビュー用コマンド3個の2章構成。
...
進捗レポートを立ち上げ・更新・公開する。手順とスキーマは Skill `progress-report` にある。

#### 引数

| 引数 | 必須 | 説明 |
|---|---|---|
| `<init\|update\|publish>` | 必須 | 実行する操作 |
| `--dir=<path>` | 任意 | データディレクトリ。未指定時は `.progress-report/<レポートID>/` |
| `--title=<name>` | 任意 | 公開時のページ名。未指定時は `meta.title` |
...
```
/progress-report publish --title=お知らせ通知機能の進捗
```

#### 注意

- `facts` は取得スクリプトの出力。手で編集しない
- `state.json` の書き換え・アーティファクトの公開は、実行前に内容を提示して承認を得る
- `git commit` / `git push` は行わない
- フラグは `=` 区切りのみ。スペース区切りは受け付けない
